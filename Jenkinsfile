...
pipeline {
    ...
    stages {
        ...
        stage('Deploy to QA') {
            ...
            post {
                success {
                    mail to: "${QA_NOTIFY}", subject: "QA Deployment Success", body: "${env.BUILD_URL}"
                }
                failure {
                    mail to: "${QA_NOTIFY}", subject: "QA Deployment Failed", body: "${env.BUILD_URL}"
                }
            }
        }
        ...
        stage('Run QA Tests') {
            ...
            post {
                success {
                    mail to: "${QA_NOTIFY}", subject: "QA Tests Passed", body: "${env.BUILD_URL}"
                    script {
                        // Merge dev -> qa
                        withCredentials([string(credentialsId: "${env.GIT_CRED_ID}", variable: 'GIT_TOKEN')]) {
                            sh '''
                              ...
                              git merge --no-ff --no-edit origin/dev || (git merge --abort && exit 1)
                              git push "https://${GIT_TOKEN}@github.com/Vedant2000039/Delphi-Pipeline-POC.git" qa
                              rm -rf "$TMPDIR"
                            '''
                        }
                    }
                ...
                failure {
                    mail to: "${QA_NOTIFY}", subject: "QA Tests Failed", body: "${env.BUILD_URL}"
                    error("QA tests failed.")
                }
            }
        }
        ...
        stage('Deploy to UAT') {
            ...
            post {
                success {
                    mail to: "${UAT_NOTIFY}", subject: "UAT Deployment Success", body: "${env.BUILD_URL}"
                    script {
                        // Merge qa -> uat
                        withCredentials([string(credentialsId: "${env.GIT_CRED_ID}", variable: 'GIT_TOKEN')]) {
                            sh '''
                              ...
                              git config user.name "Delphi CI"
                              git fetch origin qa:refs/remotes/origin/qa
                              git fetch origin uat:refs/remotes/origin/uat
                              git checkout uat || git checkout -b uat
                              git reset --hard origin/uat
                              git merge --no-ff --no-edit origin/qa || (git merge --abort && exit 1)
                              git push "https://${GIT_TOKEN}@github.com/Vedant2000039/Delphi-Pipeline-POC.git" uat
                              rm -rf "$TMPDIR"
                            '''
                        }
                    }
        ...
        stage('Deploy to PROD') {
            ...
            post {
                success {
                    mail to: "${PROD_NOTIFY}", subject: "PROD Deployment Success", body: "${env.BUILD_URL}"
                    script {
                        // Merge uat -> prod
                        withCredentials([string(credentialsId: "${env.GIT_CRED_ID}", variable: 'GIT_TOKEN')]) {
                            sh '''
                              ...
                              git merge --no-ff --no-edit origin/uat || (git merge --abort && exit 1)
                              git push "https://${GIT_TOKEN}@github.com/Vedant2000039/Delphi-Pipeline-POC.git" prod
                              rm -rf "$TMPDIR"
                            '''
                        }
                    }
