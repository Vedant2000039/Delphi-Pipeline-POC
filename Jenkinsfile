...
pipeline {
  ...
  stages {
    stage('Checkout (SCM)') {
      steps {
        // Multibranch Pipeline already checked out code for this branch.
    ...
    stage('Install Dependencies') {
      steps {
        dir('backend') {
          // Use bash explicitly for npm commands
...
# Try to update origin/qa to this exact commit (fast & exact)
set +e
...
if [ "${PUSH_EXIT}" -eq 0 ]; then
  ...
  echo "Direct push failed (protected?). Creating PR to promote commit."
  ...
  # create PR (requires jq). If jq not available, install it or ask me for jq-free variant
  ...
  echo "Please merge PR ${PR_URL} to continue deployment (branch protected)."
  exit 2
fi
BASH
''')
...
if [ "${PUSH_EXIT}" -ne 0 ]; then
  ...
  curl -s -H "Authorization: token ${GITHUB_TOKEN}" -X POST -d "${API_PAYLOAD}" "https://api.github.com/repos/Vedant2000039/Delphi-Pipeline-POC/pulls" > /tmp/pr_response_uar.json
  PR_URL=$(jq -r '.html_url' /tmp/pr_response_uar.json || echo "")
  echo "PR created: ${PR_URL}"
  exit 2
fi
BASH
...
if [ "${PUSH_EXIT}" -ne 0 ]; then
  ...
  curl -s -H "Authorization: token ${GITHUB_TOKEN}" -X POST -d "${API_PAYLOAD}" "https://api.github.com/repos/Vedant2000039/Delphi-Pipeline-POC/pulls" > /tmp/pr_response_prod.json
  PR_URL=$(jq -r '.html_url' /tmp/pr_response_prod.json || echo "")
  echo "PR created: ${PR_URL}"
  exit 2
fi
BASH
