...
pipeline {
  ...
  options { disableConcurrentBuilds(); timestamps(); buildDiscarder(logRotator(numToKeepStr: '20')) }

  stages {

    stage('Checkout') {
      steps { git branch: 'dev', url: "${env.REPO_URL}" }
    }
    ...
    stage('Unit Tests - Dev') {
      ...
      post {
        success { mail to: "${DEV_NOTIFY}", subject: "Unit Tests PASSED (Dev)", body: "${env.BUILD_URL}" }
        failure { mail to: "${DEV_NOTIFY}", subject: "Unit Tests FAILED (Dev)", body: "${env.BUILD_URL}" }
      }
    }
  ...
  steps {
    ...
    stage('Deploy to UAT') {
      ...
      steps { sh 'bash scripts/deploy.sh uat' }
    }

    stage('Deploy to PROD') {
      when { branch 'main' }
      steps { sh 'bash scripts/deploy.sh prod' }
    }
  }
